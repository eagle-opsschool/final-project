...
if ( getenv( 'MEDIAWIKI_SHOW_EXCEPTION_DETAILS' ) === 'true' ) {
...
$wgUseInstantCommons  = getenv( 'MEDIAWIKI_USE_INSTANT_COMMONS' );
...
switch ( getenv( 'MEDIAWIKI_MAIN_CACHE_TYPE' ) ) {
    case 'CACHE_ACCEL':
    ...
    case 'CACHE_MEMCACHED':
        ...
        $wgMemCachedServers = explode( ',', getenv( 'MEDIAWIKI_MEMCACHED_SERVERS' ) );
...
$tmpProxy = getenv( 'MEDIAWIKI_PROXY_SERVERS' );
if ( $tmpProxy ) {
...
$flowNamespaces = getenv( 'MEDIAWIKI_FLOW_NAMESPACES' );
if ( $flowNamespaces ) {
...
########################### VisualEditor ###########################
$tmpRestDomain = getenv( 'MEDIAWIKI_REST_DOMAIN' );
$tmpRestParsoidUrl = getenv( 'MEDIAWIKI_REST_PARSOID_URL' );
if ( $tmpRestDomain && $tmpRestParsoidUrl ) {
    ...
    $tmpRestRestbaseUrl = getenv( 'MEDIAWIKI_REST_RESTBASE_URL' );
    if ( $tmpRestRestbaseUrl ) {
        ...
        $tmpRestProxyPath = getenv( 'MEDIAWIKI_REST_RESTBASE_PROXY_PATH' );
        if ( $tmpProxy && $tmpRestProxyPath ) {
            $wgVisualEditorFullRestbaseURL = $wgServer . $tmpRestProxyPath;
        } else {
            $wgVisualEditorFullRestbaseURL = $wgServer . ':' . getenv( 'MEDIAWIKI_REST_RESTBASE_PORT' ) . "/$tmpRestDomain/";
        }
...
########################### Search Type ############################
switch( getenv( 'MEDIAWIKI_SEARCH_TYPE' ) ) {
    case 'CirrusSearch':
        # https://www.mediawiki.org/wiki/Extension:CirrusSearch
        wfLoadExtension( 'Elastica' );
        require_once "$IP/extensions/CirrusSearch/CirrusSearch.php";
        $wgCirrusSearchServers =  explode( ',', getenv( 'MEDIAWIKI_CIRRUS_SEARCH_SERVERS' ) );
        if ( $flowNamespaces ) {
